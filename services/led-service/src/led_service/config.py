...
# -> parents[0] = led_service
# -> parents[1] = src
# -> parents[2] = led-service (service root)
...
def _load_leds_config(path: Path = LEDS_CONFIG_PATH) -> LEDServiceConfig:
    ...
    The structure must match LEDServiceConfig as defined in config_schema.py.
    """
    if not path.exists():
        raise ConfigError(f"LED configuration file not found: {path}")

    try:
        raw_text = path.read_text(encoding="utf-8")
    except OSError as exc:
        raise ConfigError(f"Failed to read LED configuration file: {path}") from exc

    try:
        data = json.loads(raw_text)
    except json.JSONDecodeError as exc:
        raise ConfigError(f"Invalid JSON in LED configuration file: {path}") from exc

    return LEDServiceConfig.model_validate(data)
