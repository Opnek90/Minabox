...
`middleware/auth.py` guards path prefixes, not individual routes. The **longest
matching prefix** decides, so a specific path can sit in a stricter area than
the general prefix it lives under: `/api/v1/audio/restart-service` restarts a
container and is `admin`, while the rest of `/api/v1/audio` is `player`, which
is off by default. Resolving that by map order would have made the answer
depend on where the next entry happens to be added.

Three areas can be protected independently:
