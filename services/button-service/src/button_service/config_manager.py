...
class ConfigManager:
    ...
    - Loading the current button configuration
    - Validating and persisting new configurations
    - Notifying listeners when configuration changes
    """

    def __init__(self, config_path: Path = BUTTONS_CONFIG_PATH) -> None:
    ...
    def load_config(self) -> ButtonServiceConfig:
        """Load the button configuration from disk.
        
        Returns:
            The loaded and validated button configuration.
            
        Raises:
            ConfigError: If the configuration cannot be loaded or is invalid.
        """
        config = _load_buttons_config(self._config_path)
        self._current_config = config
        logger.info(
            "config_loaded",
            path=str(self._config_path),
            buttons_count=len(config.buttons),
        )
        return config
    ...
    def update_config(self, new_config: ButtonServiceConfig) -> None:
        """Validate and persist a new button configuration.
        ...
        Args:
            new_config: The new button configuration to apply.
        ...
        logger.info(
            "config_updated",
            path=str(self._config_path),
            buttons_count=len(new_config.buttons),
        )

        # Notify listeners
        self._notify_reload_callbacks(new_config)

    def reload_config(self) -> ButtonServiceConfig:
    ...
    def register_reload_callback(
        self, callback: Callable[[ButtonServiceConfig], None]
    ) -> None:
        ...
        Args:
            callback: A callable that accepts a ButtonServiceConfig.
        """
        self._reload_callbacks.append(callback)
        logger.debug("reload_callback_registered", callback=callback.__name__)

    def _notify_reload_callbacks(self, config: ButtonServiceConfig) -> None:
